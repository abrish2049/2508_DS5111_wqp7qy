"""Tests for bin/load_snowflake.py using mocked Snowflake connectivity."""
...
def test_load_snowflake_pipeline_ingestion_loop(monkeypatch):
    ...
    triggers table structural validation DDL, and executes safe parameter-bound
    insertions.
    """
    ...
    try:
        ...
        pytest.fail("Resource variables referenced before definition. Verify cursor init.")

    # 6. Structurally assert that the schema verification table statements occurred
    assert mock_cursor.execute.call_count >= 3, (
        "The database cursor should trigger table generation plus one call per row entry."
    )

    # Extract the distinct raw query text argument configurations executed by the mock
    ...
    assert any("CREATE TABLE IF NOT EXISTS RAW_TRANSCRIPTS" in q for q in executed_queries), (
        "The script must verify target table existence before attempting ingestion."
    )
    ...
    assert len(insert_queries) == 2, "Exactly two SQL insert invocations should occur."

    for query in insert_queries:
        assert "PARSE_JSON(%s)" in query, (
            "The engine must pass payloads safely wrapped inside PARSE_JSON."
        )
        assert "%s" in query and "f" not in query, (
            "Security Violation: Hard-coded formatting detected. Use parameter placeholders."
        )
